...
contract Registry {
    ...
    mapping(string => mapping(string => mapping(string => mapping(uint => LandDetails))))
    ...
    function AcceptRequest(uint _index, uint _reqNo) external {
        ...
        uint noOfReq = landDetalsMap[_state][_district][_city][_surveyNo]
            .noOfRequests;
        ...
        for (uint i = 0; i < noOfReq; i++) {
            ...
            uint requester_reqIndx = landDetalsMap[_state][_district][_city][
                _surveyNo
            ].requests[i].reqIndex;
    ...
    function getLandDetails(
        ...
        string memory _city,
        uint _surveyNo
    ) external view returns (address, uint256, uint, uint, uint) {
        ...
        uint mv = landDetalsMap[_state][_district][_city][_surveyNo]
            .marketValue;
    ...
    function getRequestCnt_propId(
        ...
        uint _noOfRequests = landDetalsMap[_state][_district][_city][_surveyNo]
            .noOfRequests;
